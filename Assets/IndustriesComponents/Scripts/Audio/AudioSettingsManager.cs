...
{
    public class AudioSettingsManager : VoiceComponent
    {
        ...
        public UnityEvent onInitialVolumeSet;
        ...
        private void Start()
        {
            ...
            if (onInitialVolumeSet != null) onInitialVolumeSet.Invoke();
            ...
            if (recorder != null)
            {
                ...
                try
                {
                    ...
                    {
                        recorder.MicrophoneDevice = new DeviceInfo(previousMicrophone);         // set UNITY mic by string
                    }
                }
                ...
                try
                {
                    var previousId = PlayerPrefs.GetInt("PhotonMic", -1);    // set PHOTON mic by ID. internally makes sure the microphone can be used
                    foreach (var device in photonMicEnum)
                    {
                        if (device.IDInt == previousId)
                        {
