...
namespace Fusion.Addons.DataSyncHelpers
{
    public class StreamSynchedBehaviour : NetworkBehaviour, INetworkRunnerCallbacks
    {
        ...
        void Log(string log, LogLevel level = LogLevel.Details)
        {
            if ((level & logLevel) == 0)
            {
                return;
        ...
        {
            ReliableKey key = ReliableKey.FromInts((int)Object.Id.Raw, Runner.LocalPlayer.RawEncoded, (int)Time.time, (int)(TIME_PRECISION * (Time.time - (int)Time.time)));
            return key;
        }
        ...
        public void OnReliableDataProgress(NetworkRunner runner, PlayerRef player, ReliableKey key, float progress)
        {
            ...
            else
            {
                ...
                Log($"([{objectId}, {source}, {time}] Ignoring OnReliableDataProgress: {(int)Object.Id.Raw})", LogLevel.Details);
            }
        }
        ...
        public void OnReliableDataReceived(NetworkRunner runner, PlayerRef targetPlayer, ReliableKey key, ArraySegment<byte> receivedData)
        {
            ...
            if (objectId == (int)Object.Id.Raw)
            {
                if (Runner.IsServer)
                {
                    ...
                    Log($"[{objectId}, {source}, {time}] Server received OnReliableDataReceived, relaying (local player:{Runner.LocalPlayer}): {ByteArrayTools.PreviewString(receivedData.Array)}", LogLevel.Relay);
                    foreach (var player in Runner.ActivePlayers)
                    {
                ...
                {
                    Log($"[{objectId}, {source}, {time}] OnReliableDataReceived (local player:{Runner.LocalPlayer}): {ByteArrayTools.PreviewString(receivedData.Array)}");
            ...
            {
                Log($"([{objectId}, {source}, {time}] Ignoring OnReliableDataReceived {(int)Object.Id.Raw})", LogLevel.Details);
            }
        }
